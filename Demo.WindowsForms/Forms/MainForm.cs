...
namespace Demo.WindowsForms
{
   public partial class MainForm : Form
   {
      ...
      public MainForm()
      {
            ...
            if (!GMapControl.IsDesignerHosted)
         {
            ...
            if(!Stuff.PingNetwork("pingtest.com"))
            {
               ...
               MessageBox.Show("Không có kết nối internet", "Bản Đồ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }

            // config map         
            MainMap.MapProvider = GMapProviders.OpenStreetMap;
            MainMap.Position = new PointLatLng(21.036705, 105.780577);
            MainMap.MinZoom = 0;
            MainMap.MaxZoom = 24;
            MainMap.Zoom = 9;            
            ...
            top.Markers.Add(currentMarker);
                ...
                /*
                GMapMarker myMarker = new GMarkerGoogle( new PointLatLng(21.035206, 105.805808) , GMarkerGoogleType.arrow);
                myMarker.IsHitTestVisible = false;
                top.Markers.Add(myMarker);*/

                
                SqlConnection conn = DBConnection.Instance();
                conn.Open();
                SqlCommand command = new SqlCommand("Select * from nha", conn);

                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        Console.WriteLine(" Coordinate : " + reader["lat"].ToString() + reader["lng"].ToString() );

                        GMapMarker tmpMarker = new GMarkerGoogle(new PointLatLng( Convert.ToDouble( (string) reader["lng"]) , Convert.ToDouble((string)reader["lat"]) ), GMarkerGoogleType.arrow);
                        tmpMarker.IsHitTestVisible = false;
                        tmpMarker.ToolTipMode = MarkerTooltipMode.Always;
                        tmpMarker.ToolTipText = reader["ten"].ToString();
                        top.Markers.Add(tmpMarker);
                    }
                }
               ...
               {
                  PointLatLng? pos = GMapProviders.GoogleMap.GetPoint("Vietnam, Hà Nội", out status);
                  if(pos != null && status == GeoCoderStatusCode.G_GEO_SUCCESS)
                  {
                     ...
                     myCity.ToolTipText = "Chào mừng đến với Việt Nam!";
      ...
      private void checkBoxTileHost_CheckedChanged(object sender, EventArgs e)
      {
        ...
        {
            Console.WriteLine("Click to map ");
        }
        ...
        private void button18_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = DBConnection.Instance();
            conn.Open();
            SqlCommand command = new SqlCommand("Select top 1 lng,lat from nha where ten LIKE @ten", conn);
            ...
            using (SqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    //MessageBox.Show(reader["lng"].ToString());

                    double lat = double.Parse(reader["lng"].ToString(), CultureInfo.InvariantCulture);
                    double lng = double.Parse(reader["lat"].ToString(), CultureInfo.InvariantCulture);
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (t != null)
            {
                ...
                if (t.Text.Length >= 0)
                {
                    ...
                    string connetionString = @"Data Source=THANGVQ\SQLEXPRESS;Initial Catalog=quanlydat;User ID=root3;Password=111111";
                    ...
                    string sql = "Select ten from nha ";
                    ...
                    if (reader != null)
                    {
                        while (reader.Read())
                        {
                            collection.Add(reader["ten"].ToString());
                        }
                    }
